...
const FriendRequests = ({receivedFriendRequests, sentFriendRequests, onAcceptFriendRequest, onDeclineFriendRequest, onCancelFriendRequest}: Props) => (
        ...
        <ul className={Style.list}>
            {
                sentFriendRequests.map(friendRequest =>
                    <li className={Style.entry} key={friendRequest.to.address}>
                        ...
                        {" "}<Button color="secondary" size="sm" onClick={ () => onCancelFriendRequest(friendRequest.to.address) }>Cancel</Button>
                    </li>
                )
            }
        </ul>
        ...
        <ul className={Style.list}>
            {
                receivedFriendRequests.map(friendRequest =>
                    <li className={Style.entry} key={friendRequest.from.address}>
                        <span>{friendRequest.from.username} {friendRequest.date}</span>
                        {" "}<Button color="primary" size="sm" onClick={ () => onAcceptFriendRequest(friendRequest.from.address) }>Accept</Button>
                        {" "}<Button color="secondary" size="sm" onClick={ () => onDeclineFriendRequest(friendRequest.from.address) }>Reject</Button>
                    </li>
                )
            }
        </ul>
